...
def compute_dedupe(config_path, spark, end_date, LOGGER, loaded_dob_graph):
    ...
    def md5_hash(pFirstName, pLastName, DateOfBirth):
        return hashlib.md5((pFirstName + pLastName + DateOfBirth).encode('utf-8')).hexdigest()

    md5_hash_udf = F.udf(md5_hash, StringType())

    
    df = df.withColumn("passenger_hash", md5_hash_udf(F.col("pFirstName"), F.col("pLastName"), F.col("DateOfBirth")))

    

    null_hash = hashlib.md5("CCAI_NULL".encode('utf-8')).hexdigest()

    
    df = df.withColumn("passenger_hash",
                    F.when(F.col("passenger_hash").isNull(), null_hash)
                        .otherwise(F.col("passenger_hash")))

    df.createOrReplaceTempView('df_dedupe')

    query = """
        SELECT pFirstName, pLastName, DateOfBirth, 
            FIRST(passenger_hash) AS passenger_hash, 
            COLLECT_LIST(pFirstName) AS FirstName, 
            COLLECT_LIST(pLastName) AS LastName
        FROM df_dedupe
        GROUP BY pFirstName, pLastName, DateOfBirth
    """

    lookUp_df = spark.sql(query)

    storage_path = config["storageDetails"][5]["pathUrl"].split("/")[2]
    lookUp_df_path = f"s3a://{storage_path}/dedupe_lookup/p_date={end_date}/"

    lookUp_df.write.mode("overwrite").parquet(lookUp_df_path)

    df = df.select(F.col("PassengerID"), F.col("passenger_hash"))
